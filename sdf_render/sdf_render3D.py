...
def draw_wall_solid_color(frame, rect, shading, obj):
    color = _color_.get_color_with_shading(obj.color, shading)
    pygame.draw.rect(frame, color, rect)


def draw_3D_wall_segment(frame, ray, depth, angle, obj):
    ...
    draw_wall_solid_color(frame, screen_rect, shading, obj)

    if _var_.DRAW_REFLECTION:
        _reflection_.draw_reflection(frame, screen_rect, shading, obj)
