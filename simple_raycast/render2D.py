...
def draw_2D_cell(col,row):
    ...
    pygame.draw.rect(_var_.win, cur_color, (col * _var_.TILE_SIZE, row * _var_.TILE_SIZE, _var_.TILE_SIZE - 2, _var_.TILE_SIZE - 2))
...
def draw_2D_player_base_rays(player_x,player_y,player_angle):
    ...
    pygame.draw.line(_var_.win, _color_.green, (player_x, player_y), (
        player_x - math.sin(player_angle - _var_.HALF_FOV) * 50, player_y + math.cos(player_angle - _var_.HALF_FOV) * 50), 3)
    pygame.draw.line(_var_.win, _color_.green, (player_x, player_y), (
        player_x - math.sin(player_angle + _var_.HALF_FOV) * 50, player_y + math.cos(player_angle + _var_.HALF_FOV) * 50), 3)
...
def draw_2D_rays(col, row, target_x, target_y,player_x,player_y):
    if not _var_.draw2D: return
    pygame.draw.rect(_var_.win, _color_.green, (col * _var_.TILE_SIZE, row * _var_.TILE_SIZE, _var_.TILE_SIZE - 2, _var_.TILE_SIZE - 2))
